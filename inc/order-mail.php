...
<?php
	global $wpdb;			
	$email_body_cilent = get_option( 'cleanup_order_client_email');
	$order_email_subject_client = get_option( 'cleanup_order_client_email_sub');			
	...
	$invoice_no='000-'.$userId.'-'.gmdate('Y',time());			
	...
	$dis_amount =get_user_meta($userId, 'cleanup_discount',true);
	
	$package_amount_2=$cleanup_package_cost.' '.$api_currency;
	$discount_2= $dis_amount.' '.$api_currency;
	$total_g =($cleanup_package_cost - $dis_amount).' '.$api_currency;
	...
	// For Admin Email*****	
	$email_body_admin = get_option( 'cleanup_order_admin_email');
	$order_email_subject_admin = get_option( 'cleanup_order_admin_email_sub');			
	...
	$trial_enable= get_post_meta( $package_id,'cleanup_package_enable_trial_period',true); 
	if( $trial_enable!='yes' ){	
