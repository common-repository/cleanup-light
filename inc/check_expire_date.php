...
<?php
	...
	$payment_gateway=get_user_meta($userId, 'cleanup_payment_gateway', true);
		...
		if($exprie_date < $current_date){
			...
			if(isset($recurringCheck['STATUS'])){
				if($recurringCheck['STATUS']=='Active'){
				   ...
				   $wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}posts SET post_type=%s WHERE id=%s LIMIT 1", $post_type,$newpost_id ));
					$wpdb->query($query);
	...
	if($payment_gateway=='stripe'){
		...
		$payment_status=get_user_meta($userId, 'cleanup_payment_status', true);	
		if($payment_status=='pending'){	
			require_once(cleanup_ep_DIR . '/admin/files/lib/Stripe.php');						
