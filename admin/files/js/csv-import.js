...
function cleanup_update_csv_file(){ 
		"use strict";
	   var ajaxurl = dirpro_data.ajaxurl;		  	  
	   var search_params={
		 "action"  :  "cleanup_finalerp_csv_product_upload",		
...
function cleanup_save_csv_file_to_database(){
	"use strict";
	cleanup_upload_status_checker();
	...
	jQuery("#ep3").show();
	var search_params2={
		"action"  :  	"cleanup_save_csv_file_to_database",	
	...
	jQuery.ajax({
		 ...
		 success : function(response){	
			...
			if(response.code=='not-done'){ 
				...
				cleanup_save_csv_file_to_database(); 	
...
function cleanup_upload_status_checker(){
	"use strict";
	var importTimer;		
	...
	importTimer = setInterval(function() {
		// Get the current status of the update
			...
			var search_params3={
					"action"  :  "cleanup_eppro_get_import_status",					
			};
			jQuery.ajax({
				 url : ajaxurl,
				 dataType : "json",
...
function cleanup_upload_csv_file(update_div_id){
	"use strict";
