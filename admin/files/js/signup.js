...
function cleanup_epluginrecaptchaSubmit(token){
	"use strict";		
		
	var formc = jQuery("#cleanup_registration");
	if (jQuery.trim(jQuery("#iv_member_user_name",formc).val()) == "" || jQuery.trim(jQuery("#iv_member_email",formc).val()) == "" || jQuery.trim(jQuery("#iv_member_password",formc).val()) == "") {	
		...
		if( dirpro_data.iv_gateway=='stripe'){
			...
			}, cleanup_stripeResponseHandler);
						
		}else{ /* Else for paypal */
				
				jQuery('#cleanup_registration').on( "submit");		
...
(function($) {	
	...
	jQuery(document).ready(function($) {
	 ...
	 // this identifies your website in the createToken call below	
			...
			function cleanup_stripeResponseHandler(status, response) {
				if (response.error) {				
					...
					var form$ = jQuery("#cleanup_registration");
...
function cleanup_show_coupon(){
	"use strict";
	jQuery("#coupon-div").show();
	 jQuery("#show_hide_div").html('<label for="text" class="col-md-4 col-xs-4 col-sm-4 control-label"></label><div class="col-md-8 col-xs-8 col-sm-8 " ><button type="button" onclick="cleanup_hide_coupon();"  class="btn btn-small-ar center">'+dirpro_data.HideCoupon+'</button></div>');
}
function cleanup_hide_coupon(){
	"use strict";
	 jQuery("#coupon-div").hide();
	 jQuery("#show_hide_div").html('<label for="text" class="col-md-4 col-xs-4 col-sm-4 control-label"></label><div class="col-md-8 col-xs-8 col-sm-8 " ><button type="button" onclick="cleanup_show_coupon();"  class="btn btn-small-ar center">'+dirpro_data.Havecoupon+'</button></div>');
}