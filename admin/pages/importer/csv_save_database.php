...
<?php
	...
	if($field_set!=""){ 
		...
		$default_fields['other_link']=esc_html__('Other Link','cleanup');
	}
		...
		while (($data = fgetcsv($handle)) !== FALSE) {
			if($iii>=$row  AND $row<$row_max){	
					...
					if(in_array("ID", $top_header) OR in_array("Id", $top_header) OR in_array("id", $top_header)){
						...
						if(strtolower($top_header[$i])=='id'){		
							if(trim($one_col)!=''){
								...
								if ( isset($post_check->post_type) and $post_check->post_type==$cleanup_directory_url ) {								
									$post_id=$one_col;	
									}else{
									$post_id=0;								
								}							
				...
				if(isset($post_data['category'])) {  
					...
					wp_set_object_terms( $post_id, $post_cat_arr, $cleanup_directory_url.'-category');
				}				
				if(isset($post_data['tag'])) { 
					$post_tag_arr =explode(",",$post_data['tag']) ;
					wp_set_object_terms( $post_id, $post_tag_arr, $cleanup_directory_url.'-tag');
				}
				if(isset($post_data['locations'])) {  
					$post_location_arr =explode(",",$post_data['locations']) ;
					wp_set_object_terms( $post_id, $post_location_arr, $cleanup_directory_url.'-locations');
				}
				...
				$row++;
				update_option( 'cleanup_current_row',$row);	
			}		
			$iii++;	
		}
	}
	$row_done=$row_max;
	if($row_max >=$cleanup_total_row){$done_status='done';}
	fclose($handle);
?>