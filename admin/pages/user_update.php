...
?>
		<div class=" card col-md-12">
			<div class="row">
				<div class="col-md-12"><h3 class=""><?php esc_html_e( 'User Update: ', 'cleanup' );?>
				<?php 
				...
				<a  href="<?php echo esc_url($page_link); ?>" target="_blank" class="btn btn-info btn-xs ">
					<?php esc_html_e('Profile','cleanup'); ?> </a>
					 </h3>
				</div>	
			</div> 
			<div class="card-body ">				
				<form id="user_form_iv" name="user_form_iv" class="form-horizontal" role="form" onsubmit="return false;">				
					...
					<div class="form-group row">
						<label for="inputEmail3" class="col-md-4 control-label"><?php esc_html_e( 'User Name', 'cleanup' );?></label>
						<div class="col-md-8">
							<label for="inputEmail3" class="control-label"><?php echo esc_html($user->user_login); ?></label>
						</div>
					</div>
					<div class="form-group row">
						<label for="inputEmail3" class="col-md-4 control-label"><?php esc_html_e( 'Email Address', 'cleanup' );?></label>
						<div class="col-md-8">									
							<label for="inputEmail3" class="control-label"><?php echo esc_html($user->user_email); ?></label>
						</div>
					</div>								 
					<div class="form-group row">
						<label for="text" class="col-md-4 control-label"><?php esc_html_e( 'User Role', 'cleanup' );?></label>
						<div class="col-md-8">
							<?php
								$user_role= '';
					...
					<div class="form-group row">
						<label for="text" class="col-md-4 control-label"><?php esc_html_e( 'User Package', 'cleanup' );?></label>
						<div class="col-md-8">									
							<?php
								$post_type='cleanup_pack';
								...
								if($membership_pack>0){
									$i=0; $current_package_id=get_user_meta($user_id,'cleanup_package_id',true);
								echo'<select name="package_sel"  class=" form-control">'; ?>
								<option value="" ><?php esc_html_e( 'Select Package', 'cleanup' );?></option>
								<?php
									foreach ( $membership_pack as $row )
									{
										if($current_package_id==$row->ID){
											echo '<option value="'. esc_attr($row->ID).'" selected>'. esc_html($row->post_title). esc_html__( '[User Current Package]', 'cleanup' ).' </option>';
											}else{
					...
					<div class="form-group row">
						<label for="text" class="col-md-4 control-label"><?php esc_html_e( 'Payment Status', 'cleanup' );?></label>
						<div class="col-md-8">
							<?php
								$payment_status= get_user_meta($user_id, 'cleanup_payment_status', true);
							?>
							<select name="payment_status" id ="payment_status" class="form-control">
								<option value="success" <?php echo ($payment_status == 'success' ? 'selected' : '') ?>><?php esc_html_e( 'Success', 'cleanup' );?></option>
								<option value="pending" <?php echo ($payment_status == 'pending' ? 'selected' : '') ?>><?php esc_html_e( 'Pendinge', 'cleanup' );?></option>
								</select>	
						</div>
					</div>
					
					<div class="form-group row">
						<label for="inputEmail3" class="col-md-4 control-label"><?php esc_html_e( 'Expiry Date', 'cleanup' );?></label>
						<div class="col-md-8">
							<?php
					...
					<div class="form-group row">
						<label for="inputEmail3" class="col-md-4 control-label"><?php esc_html_e( 'Profile Image', 'cleanup' );?></label>
						<div class="col-md-2">
								...
								<div class="avatar" id="profile_image_main">
									<?php
										...
										<?php
											...
											echo'	 <img class="rounded-profileimg" src="'. cleanup_ep_URLPATH.'assets/images/default-user.png">';
										}
									?>
								</div>
							</div>	
						...
						<div class="col-md-6">
							...
							<div class="btn-upload">
								<button type="button" onclick="cleanup_edit_profile_image('profile_image_main');"  class="btn btn-info ">
								<?php esc_html_e('Change Image','cleanup'); ?> </button>
					...
					<div class="form-group row">
						<label for="inputEmail3" class="col-md-4 control-label"><?php esc_html_e( 'Profile Baner Image', 'cleanup' );?></label>
						<div class="col-md-2">
						...
						<div class="col-md-6">
							<div class="upload">
							<div class="btn-upload">							
								<button type="button" onclick="cleanup_edit_banner_image('banner_image_main');"  class="btn btn-info ">
		<?php esc_html_e('Change Banner [best fit: 1200 X 400]','cleanup'); ?> </button>
							</div>
							</div>
						</div>
					...
					<div class="form-group row">
						...
						<div class="row">
							<?php
								...
								$field_set=get_option('cleanup_profile_fields' );
								if($field_set!=""){
								...
								foreach ( $default_fields as $field_key => $field_value ) { 		
									if(isset($myaccount_fields_array[$field_key])){  
										if($myaccount_fields_array[$field_key]=='yes'){ 
											...
											if($role_access=='yes'){
												echo  $main_class->cleanup_check_field_input_access($field_key, $field_value, 'myaccount', $user_id );
											}
										}
										}else{ 
										echo  $main_class->cleanup_check_field_input_access($field_key, $field_value, 'myaccount', $user_id);
					...
					<div class="col-md-12" id="usersupdatemessage"></div>
						<div class="col-md-12">							
							<label for="" class="col-md-4 control-label"></label>							
							<div class="col-md-8">
								<button class="btn btn-info " onclick="return cleanup_update_user_setting();"><?php esc_html_e( 'Update User', 'cleanup' );?></button>
								<a href="<?php echo cleanup_ep_ADMINPATH; ?>admin.php?page=cleanup-settings&user_settings" class="btn btn-info" ><?php esc_html_e( '<< Back', 'cleanup' );?></a>
							</div>
							<p>&nbsp;</p>
						</div>
					</div>
