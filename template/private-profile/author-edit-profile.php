...
<div class="row"> 
	<div class="col-md-4">			
		<div class="upload-avatar">
			<div class="avatar" id="profile_image_main">
				<?php
					$iv_profile_pic_url=get_user_meta($current_user->ID, 'cleanup_profile_pic_thum',true);
					if($iv_profile_pic_url!=''){ ?>
					<img class="img-fluid"  src="<?php echo esc_url($iv_profile_pic_url); ?>">
					<?php
						}else{
						echo'	 <img class="img-fluid"  src="'. cleanup_ep_URLPATH.'assets/images/company-enterprise.png">';
					}
				?>
			</div>
		</div>
	...
	<div class="col-md-8">
		<button type="button" onclick="cleanup_edit_profile_image('profile_image_main');"  class="btn btn-small-ar">
		<?php esc_html_e('Change Logo','cleanup'); ?> </button>
	</div>
	<div class="col-md-4 mt-3">	
		...
		<div class="avatar" id="banner_image_main">
			<?php					
				echo'<img class="img-fluid" src="'. esc_url($default_image_banner).'">';
			?>
		</div>
	</div>		
	<div class="col-md-8 mt-3">	
		<button type="button" onclick="cleanup_edit_banner_image('banner_image_main');"  class="btn btn-small-ar">
	...
	<div class="col-md-12 mt-3">
		<div class="form-group">
			<label class=""><?php esc_html_e('Industry', 'cleanup'); ?></label>
	...
	<div class="col-md-12 mt-3">
		<div class="form-group">		
			<label class=""><?php esc_html_e('Location', 'cleanup'); ?></label>
			<hr/>
			<div class=" row">		
			<?php
				$args =array();					
			...
			?>
				<div class="col-md-12">
					<input type="text" class="form-control" name="new_location" id="new_location" value="<?php echo get_user_meta($current_user->ID,"new_locations",true); ?>" placeholder="<?php  esc_html_e('Enter New Locations: Separate with commas','cleanup'); ?>">
				</div>	
		...
		foreach ( $default_fields as $field_key => $field_value ) { 		
			if(isset($myaccount_fields_array[$field_key])){  				
				...
				if($myaccount_fields_array[$field_key]=='yes'){ 
					...
					if($role_access=='yes'){ 
						echo  $main_class->cleanup_check_field_input_access($field_key, $field_value, 'myaccount', $current_user->ID );
					}
				}
			}else{ 
				echo  $main_class->cleanup_check_field_input_access($field_key, $field_value, 'myaccount', $current_user->ID );
...
<div class="margin-top-10">
	...
	<button type="button" onclick="cleanup_update_profile_setting();"  class="btn green-haze"><?php   esc_html_e('Save Changes','cleanup');?></button>
</div>