...
<div class="row form-group">
	<label for="text" class="col-md-4"><?php  esc_html_e('Package Name','cleanup');?></label>
	<div class="col-md-8">
		<?php
			$recurring_text='';
			...
			if( $package_name==""){
				...
				if(is_array($membership_pack)){
					...
					if($recurring == 'on'){
						$package_amount=get_post_meta($package_id, 'cleanup_package_recurring_cost_initial', true).' '.$api_currency.'/'.get_post_meta($package_id, 'cleanup_package_recurring_cycle_count', true).' '.get_post_meta($package_id, 'cleanup_package_recurring_cycle_type', true);
						}else{
						$package_amount=get_post_meta($package_id, 'cleanup_package_cost',true).' '.$api_currency;
					}
				?>
				<?php
				}
...
<div class="row form-group">
	<label for="text" class="col-md-4"><?php  esc_html_e('Amount','cleanup');?></label>
...
<div class="row">
	...
	<div class="col-md-8 ">
		...
		if($eprecaptcha_api==''){
			?>
				<button  id="submit_cleanup_payment" name="submit_cleanup_payment"  type="submit" class="btn btn-secondary"  > <?php  esc_html_e('Submit','cleanup');?></button>	
		<?php
		}else{
		?>
