	...
	<div class="row align-items-center">
		<div class="col-md-5">
			<?php			
				...
				?>
				<button onclick="cleanup_apply_popup('<?php echo esc_attr($listingid);?>')" class="btn btn-big mr-2 mb-2 ">
					<?php 
					...
					?>
				<?php esc_html_e('Apply Now','cleanup'); ?></button>	
				<?php
				}
				...
				if(array_key_exists('claim_button',$active_single_fields_saved)){ 
					 ?>
						<button type="button" class="btn btn-border mt-1 mb-2 " onclick="cleanup_claim_popup('<?php echo esc_html($listingid);?>')"><?php esc_html_e( 'Report', 'cleanup' ); ?></button>
					<?php
