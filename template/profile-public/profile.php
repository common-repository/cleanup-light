...
<?php
	...
	wp_enqueue_style('cleanup-profile-public', cleanup_ep_URLPATH . 'admin/files/css/profile-public.css');
	wp_enqueue_style('all-awesome', cleanup_ep_URLPATH . 'admin/files/css/all.min.css');
		...
		<section class="section ">   		
			...
			<div class="row mt-2">
				<div class="col-lg-7 col-md-12 ">					
					<h2 class="title-detail mr-2"><?php echo (get_user_meta($user_id,'full_name',true)!=''? get_user_meta($user->ID,'full_name',true) : $user->display_name ); ?>
						<?php					
			...
			<div class="col-lg-5 col-md-12 text-lg-end ">
				<div class="btn-feature text-right">						
					<button class="btn btn-big  mb-2" onclick="cleanup_author_email_popup('<?php echo esc_attr($user_id);?>')">
					<?php esc_html_e('Contact Us', 'cleanup'); ?></button>
					<?php
						$current_ID = get_current_user_id();
						$favourites='no';
						if($current_ID>0){
							$my_favorite = get_post_meta($user_id,'cleanup_authorbookmark',true);											
							$all_users = explode(",", $my_favorite);
							if (in_array($current_ID, $all_users)) {
								$favourites='yes';
							}
						}
						$added_to_Boobmark=esc_html__('Saved', 'cleanup');
						$add_to_Boobmark=esc_html__('Save', 'cleanup');
					?>
	...
	<div class="row mt-5">
		<div class="col-lg-8 col-md-12 col-sm-12 col-12 profile-col1">
			<div class="listing-overview">					
				<div class="border-bottom pb-15 mb-3 toptitle"><?php esc_html_e('About', 'cleanup'); ?>
				</div>
				...
				<div class="border-bottom pb-15 mb-3 toptitle"><?php esc_html_e('Industry', 'cleanup'); ?>
				</div>			
			...
			<?php
				include(cleanup_ep_template . '/profile-public/footer_share.php');  
			?>
		</div>
		<div class="col-lg-4 col-md-12 col-sm-12 col-12 pl-40 pl-lg-15 mt-lg-30 profile-col2">
			<div class="sidebar-border">
				...
				<div class="sidebar-list-listing">
					...
					<ul class="ul-disc ml-3">
						...
						?>
						<?php if($email!=''){  ?>
							<li><?php esc_html_e('Email','cleanup'); ?> : <?php echo esc_html($email); ?></li>
							<?php
							}
						?>
						<?php if($company_web!=''){  ?>
			...
			<?php
				include(cleanup_ep_template.'/profile-public/author-listings.php');			
			?>
		</div>
	</div>		
</div>
...
<?php
	...
	wp_enqueue_script('epmyaccount-script-27', cleanup_ep_URLPATH . 'admin/files/js/public-profile.js');
	wp_localize_script('epmyaccount-script-27', 'cleanup1', array(
	...
	'Add_to_Boobmark'=>esc_html__('Add to Boobmark', 'cleanup' ),
	'Added_to_Boobmark'=>esc_html__('Added to Boobmark', 'cleanup' ),	
	) );
