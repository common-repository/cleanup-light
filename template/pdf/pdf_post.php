...
<?php
	if(isset($_REQUEST['cleanuppdfpost'])){ 
		global $html_pdf;
		global $current_user;
		...
		ob_clean();
		require ( cleanup_ep_ABSPATH. 'inc/vendor/autoload.php');
		$user_id=1;
		if(isset($_REQUEST['cleanuppdfpost'])){
			$author_name= sanitize_text_field($_REQUEST['cleanuppdfpost']);
		...
		$footer_html='';
		$postid=sanitize_text_field($_REQUEST['cleanuppdfpost']);	
		$listingid=$postid;
		...
		if(isset($feature_image[0])){
			...
			$feature_img= cleanup_ep_URLPATH."assets/images/listing.png";
		}
		...
		<td scope="row" style="text-align: left;width:50%"><h2>'. $name_display .'</h2><br/>'.$address.' <br/> '.$client_email_address.'<br/>'. esc_html__('Phone','cleanup').' : '.esc_attr($phone).'  <br/> '. esc_html__('Web','cleanup').' : '.$web.'</td>	
		...
		<tr><td><h4>'. esc_html__('listing Summary','cleanup').'</h4><hr></td></tr>		
		<tr><td>	'.esc_html__('Published','cleanup').' : '.get_the_date('M d, Y', $listingid).'<br></td></tr>		
		';
		$html_pdf=$html_pdf.'<tr >		
		<td scope="row" style="text-align: left;width:100%,"><br/><h4>'. esc_html__('listing Description','cleanup').'</h4><hr>'.$dir_detail->post_content.'</td>
		</tr>';		 
		$cleanup_fields=  		get_option( 'cleanup_li_fields' );
		if(is_array($cleanup_fields)){
			foreach ( $cleanup_fields as $field_key_pass => $field_value ) { 
		...
		$stylesheet = file_get_contents(cleanup_ep_URLPATH . 'admin/files/css/pdf.css');		
